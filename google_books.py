...
Metadata lookups against Google Books AND OpenLibrary, plus three features:
  ...
  4. FIX: build_lazy_load_trigger's background search now also runs
     concurrently (same 4-worker pattern as fetch_missing_metadata_parallel)
     instead of looking up one book at a time, and saves the library
     once at the end instead of after every single book. This is what
...
def build_lazy_load_trigger(
    ...
    Returns an ElevatedButton. On click it:
      ...
      3. Kicks off a background thread that looks up each missing book
         CONCURRENTLY (up to 4 at a time, same pattern as
         fetch_missing_metadata_parallel) via search_book_metadata
         (Google Books, then OpenLibrary for whatever's still missing),
         instead of looking them up one at a time.
      4. As each result comes back, swaps that book's ghost placeholder
         for the real card produced by `render_book_card`.
      5. Saves the library once, after every lookup has finished,
         instead of writing to disk after every single book.
    """
